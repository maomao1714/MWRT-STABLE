...
if [ ! -f .config ]; then
    echo ">>> [ERROR] .config 不存在，无法修改 QModem 配置"
    exit 1
fi
...
echo ">>> [QModem] 配置完成"
echo ">>> [QModem] 当前配置："
grep -E '^CONFIG_PACKAGE_(qmodem|luci-app-qmodem|sms-forwarder-next|quectel-CM-5G-M)' .config || true
...
if [ ! -d feeds ] && [ ! -d package ]; then
    echo ">>> [WARN] feeds 和 package 目录均不存在，跳过 APN 补丁"
    _MODEM_DIAL=""
else
...
if [ -z "$_MODEM_DIAL" ]; then
    ...
    if ! command -v python3 >/dev/null 2>&1; then
        echo ">>> [WARN] python3 不可用，跳过 APN 补丁"
    elif grep -q "auto_detect_apn" "$_MODEM_DIAL"; then
...
if anchor1 not in content:
    print('WARN: at_dial() not found, skipping function insertion', file=sys.stderr)
else:
    content = content.replace(anchor1, FUNC + anchor1, 1)

anchor2 = 'if [ "$pdp_index" = "3" ];then\n'
if anchor2 not in content:
    print('WARN: pdp_index=3 pattern not found, skipping APN check insertion', file=sys.stderr)
else:
...
PYEOF
        if python3 /tmp/patch_modem_dial.py "$_MODEM_DIAL"; then
            echo ">>> [OK] APN 补丁成功"
            echo ">>> 补丁内容验证："
            grep -n "auto_detect_apn" "$_MODEM_DIAL" | head -5
        else
            echo ">>> [WARN] APN 补丁未完全应用，但脚本继续执行"
        fi
...
start_instance() {
    local cfg="$1"
    local enabled listen_port db_path base_path admin_username admin_password
    local bin_path web_path music_dir
...
o.rawhtml = true
if is_running then
    o.value = 'SongLoft 运行中 ' .. '【打开管理界面 192.168.1.1:58091】'
else
    o.value = 'SongLoft 未运行'
end
...
while [ "$count" -lt 20 ]; do
    if [ -d /sys/class/ieee80211/phy0 ]; then break; fi
    sleep 1
    count=$((count + 1))
done
...
# ---- 历史 WED 强制删除检查 ----
if grep -R -E 'patches-6\.18.*(941|942|943|944|945|946|947|948|949)' diy-part1.sh diy-part2.sh 2>/dev/null; then
...
# ---- 旧 wifi-scripts 下载检查 ----
if grep -R -F 'openwrt/openwrt/v24.10.5/package/network/config/wifi-scripts' diy-part1.sh diy-part2.sh 2>/dev/null; then
...
# ---- OpenVPN DCO 检查 ----
if grep -E -n 'ovpn-dco|ENABLE_DCO|disable.*DCO' diy-part1.sh diy-part2.sh 2>/dev/null; then
...
if [ -n "$_MODEM_DIAL_CHECK" ]; then
    if grep -q "auto_detect_apn" "$_MODEM_DIAL_CHECK"; then
        ...
        echo ">>> [WARN] modem_dial.sh 未包含 APN 补丁（可能是锚点不匹配，不影响编译）"
    fi
else
    echo ">>> [WARN] 未找到 modem_dial.sh，跳过 APN 补丁检查"
fi

# ---- WH3000 / WH3000 Pro WiFi 检查 ----
if [ "$DEVICE" = "wh3000" ] || [ "$DEVICE" = "wh3000pro" ]; then
    echo ""
    echo ">>> MT7981 WiFi 检查..."
    grep -E '^CONFIG_PACKAGE_(kmod-mac80211|kmod-cfg80211|kmod-mt76-core|kmod-mt76-connac|kmod-mt7915e|kmod-mt7981-firmware|mt7981-wo-firmware|wireless-regdb|iwinfo|rpcd-mod-iwinfo)=' .config || true
    echo ""
    echo ">>> 检查 LEDE 当前 wifi-scripts..."
    if [ -f package/network/config/wifi-scripts/files/lib/netifd/netifd-wireless.sh ]; then
...
if [ "$DEVICE" = "re-sp-01b" ]; then
    echo ""
    echo ">>> RE-SP-01B 配置检查..."
    grep -E '^CONFIG_TARGET_ramips_mt7621_DEVICE_jdcloud_re-sp-01b=y' .config || true
...
[ -d package/msd_lite ] && echo " ✓ msd_lite"
[ -d package/luci-app-iptv-manager ] && echo " ✓ luci-app-iptv-manager"
[ -d feeds/rtp2httpd ] && echo " ✓ rtp2httpd feed"

# ---- Docker 检查 ----
if [ "$DEVICE" = "wh3000" ] || [ "$DEVICE" = "wh3000pro" ]; then
    echo ""
    echo ">>> Docker 配置检查..."
    grep -E '^CONFIG_PACKAGE_(dockerd|docker|docker-compose|luci-app-dockerman)=' .config || true
fi
...
[ -d package/msd_lite ] && echo " ✓ msd_lite"
[ -d package/luci-app-iptv-manager ] && echo " ✓ luci-app-iptv-manager"
[ -d package/luci-app-openclash ] && echo " ✓ OpenClash"
[ -d package/songloft ] && echo " ✓ SongLoft"
[ -d package/luci-app-songloft ] && echo " ✓ luci-app-songloft"
[ -d package/luci-app-webdav ] && echo " ✓ WebDAV"
